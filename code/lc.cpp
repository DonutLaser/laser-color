...
void app_update (lc_memory* memory, lc_input input) {
	...
	opengl_clear (app -> client_width, app -> client_height, clear_color);
		...
		lc_color temp_color = make_colorb (255, 255, 255);
		lc_rect temp_rect = { };
		temp_rect.width = app -> client_width;
		temp_rect.height = app -> client_height;
		temp_rect.x = 0;
		temp_rect.y = app -> client_height;
		opengl_rect (temp_rect, temp_color, app -> image);
